...
/***/ function(module, exports, __webpack_require__) {
	...
	function startGame(event) {
	  ...
	  if (event.keyCode == 13 && !startNewGame) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.HIGH_SCORE = Game.HIGH_SCORE || 0;
	Game.LEVEL = Game.LEVEL || 1;
	Game.CRASH = Game.CRASH || 0;
	Game.DEADTOAD = Game.DEADTOAD || [];
	...
	Game.prototype.addDeadToad = function (pos) {
	  Game.DEADTOAD = pos;
	};
	...
	Game.prototype.drawCrash = function (ctx) {
	  ...
	  if (toad.length > 0) {
	    ...
	    if (toad[1] < 345 && toad[1] >= 120) {
	      crash.src = './images/splash.png';
	      imgWidth = 400;
	    ...
	    ctx.drawImage(crash, Game.CRASH, 0, 40, 80, toad[0], (toad[1] - 45), 40, 80);
	    if (Game.CRASH < imgWidth) {
	      Game.CRASH += 40;
	    } else {
	      Game.CRASH = 0;
	      Game.DEADTOAD = [];
	...
	Game.prototype.drawScore = function (ctx) {
	  ...
	  ctx.fillText("HIGH SCORE", 300, 25);
	  ctx.fillText(Game.HIGH_SCORE, 485, 25);
	};
	...
	Game.prototype.drawTimer = function (ctx) {
	  ...
	  ctx.fillRect(180, 670, 400, 20);
	
	  ctx.fillStyle = "#000000";
	  ctx.fillRect(180, 670, timeBar, 20);
	
	  ctx.fillStyle = Game.FONT_COLOR;
	  ctx.font = "26px sans-serif";
	  ctx.fillText("TIME", 590, 690);
	};
	...
	Game.prototype.gameOver = function (ctx) {
	  ...
	  ctx.drawImage(gameOverScreen, 0, 0, Game.DIM_X, Game.DIM_Y);
	
	  Toad.JUMP_COUNTER = 0;
	  Toad.JUMP_SCORE = 0;
	
	  document.addEventListener("keypress", function(event) {
	    if (event.keyCode == 32 && Game.GAME_OVER) {
	      Game.LIVES = 3;
	      Game.LEVEL = 1;
	      Game.SCORE = 0;
	      Game.DEADTOAD = [];
